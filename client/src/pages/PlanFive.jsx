...
import qrCode from '../assets/qr-45.jpg';
  ...
  const increaseAnonPostLimit = useCallback(async () => {
    try {
      ...
      const res = await fetch('http://localhost:5000/api/users/me', {
        ...
        body: JSON.stringify({
          anonPostLimit: user?.anonPostLimit + 5,
        }),
      });
      ...
      if (res.ok) {
        setUser((prevUser) => ({
          ...prevUser,
          anonPostLimit: prevUser.anonPostLimit + 5,
        }));
        console.log("Successfully incremented anonPostLimit by 5");
      } else {
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        <div style={styles.left}>
          <h2>Payment for 5 Anonymous Posts</h2>
          <p><strong>User:</strong> {user?.username || 'Loading...'}</p>
          <p><strong>Email:</strong> {user?.email || 'Loading...'}</p>
          <p><strong>Amount:</strong> ₹45</p>
